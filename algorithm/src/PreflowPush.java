...
public class PreflowPush {
	...
	private static ResidualVertex getActiveNode() {
		...
		while (vertices.hasNext()) {
			...
			if (!vertex.getName().equalsIgnoreCase("s") && !vertex.getName().equalsIgnoreCase("t")
					&& vertex.getExcess() > 0) {
				return vertex;
			}
		}
	...
	private static void push(ResidualVertex residualVertex, ResidualEdge forwardEdge) {
		...
		residualVertex.setExcess(residualVertex.getExcess() - pushValue); // set Excess
